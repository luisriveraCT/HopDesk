...
# Company registry — initials → display name
# Add or remove companies here; nothing else needs to change
COMPANY_MAP <- c(
  NG  = "Networks Group",
...
# Only these 2 are needed before any browser connects (SAP snapshot seed).
...
calendar_html <- function(
  ...
  # Ensure abono columns exist (graceful fallback when pipeline didn't add them)
  if (!"abono_total"    %in% names(dcur)) dcur$abono_total    <- 0
  if (!"has_abono"      %in% names(dcur)) dcur$has_abono      <- FALSE
  if (!"Saldo_original" %in% names(dcur)) dcur$Saldo_original <- dcur$Importe
  ...
  # We join against dcur to find which visible invoices are staged
  staged_dates <- character(0)
  staged_count_by_day <- list()
  if (!is.null(staged_keys) && nrow(staged_keys) > 0 && nrow(dcur) > 0) {
    # dcur has Fecha column from the filter above but we need it from data_due
    # Join staged keys against the full filtered data to get their dates
    staged_in_view <- if (all(c("Empresa","Documento") %in% names(data_due))) {
  data_due |>
    mutate(Moneda = toupper(trimws(Moneda))) |>
  ...
  render_tile <- function(fecha, is_weekend) {
    ...
    body_html <- if (has_data) {
      ...
      rows_html <- vapply(seq_len(nrow(shown)), function(i) {
        p        <- shown[i, ]
        has_ab   <- isTRUE(p$has_abono) && !is.na(p$has_abono)
        amt_cls  <- if (has_ab) ' cal-amount--partial' else ''
        tip_attr <- if (has_ab) sprintf(
          paste0(' data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="top"',
                 ' title="&lt;b&gt;Original:&lt;/b&gt; $%s&lt;br&gt;&lt;b&gt;Abono:&lt;/b&gt; \u2212$%s&lt;br&gt;&lt;b&gt;Neto:&lt;/b&gt; $%s"'),
          fmt(p$Saldo_original), fmt(p$abono_total), fmt(p$Importe)
        ) else ''
        icon_html <- if (has_ab) '<span class="cal-abono-icon" title="">\u2296</span>' else ''
        paste0(
          '<div class="cal-row">',
            '<span class="cal-party">', str_trunc(p$Parte, 18), '</span>',
            '<span class="cal-amount', amt_cls, '"', tip_attr, '>',
              icon_html, '$ ', fmt(p$Importe),
            '</span>',
          '</div>'
        )
      }, character(1))
